...
try:
    ...
    # в переменную, находим элемент по css селетору с ID:user-name и в это поле вписываем "standard_user"
    input_login = driver.find_element(By.CSS_SELECTOR, "#user-name").send_keys("standard_user")

    input_password = driver.find_element(By.CSS_SELECTOR, "#password").send_keys("secret_sauce")
...
# по первому заданию на заполнение полей
...
# по второму заданию про калькулятор

    # создаем список и по нему поочереди нажимаем на кнопки
    buttons = ["7", "+", "8", "="]
    for button in buttons:
        WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable((By.XPATH, f"//span[text()='{button}']"))).click() # еще пример запроса //button[text()='Gold']
